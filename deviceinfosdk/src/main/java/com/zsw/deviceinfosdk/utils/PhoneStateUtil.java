...
public class PhoneStateUtil {
    ...
    public static CustomPhoneState getPhoneState(boolean needDefaultValue) {
        ...
        try {
            String androidId = Settings.Secure.getString(DeviceInfoSDK.getInstance().getContext().getContentResolver(), Settings.Secure.ANDROID_ID);
            customPhoneState.setAndroidId(androidId);
            customPhoneState.setDeviceType(isPad() ? 5 : 4);

            int per = ContextCompat.checkSelfPermission(DeviceInfoSDK.getInstance().getContext(), Manifest.permission.READ_PHONE_STATE);
            if (per == PackageManager.PERMISSION_GRANTED) {
                TelephonyManager tm = (TelephonyManager) DeviceInfoSDK.getInstance().getContext().getSystemService(Context.TELEPHONY_SERVICE);
                String imsi = null;
                String iccid = null;
                String imei = null;
            ...
            getImeiAndMeid(DeviceInfoSDK.getInstance().getContext(), customPhoneState);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static boolean isPad() {
        return (DeviceInfoSDK.getInstance().getContext().getResources().getConfiguration().screenLayout
