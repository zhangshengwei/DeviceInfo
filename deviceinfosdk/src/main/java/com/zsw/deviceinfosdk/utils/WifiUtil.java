...
public class WifiUtil {
    ...
    public static CustomWifiInfo getWifiInfo() {
        ...
        try {
            int permission = ContextCompat.checkSelfPermission(DeviceInfoSDK.getInstance().getContext(), Manifest.permission.ACCESS_WIFI_STATE);
            if (permission != PackageManager.PERMISSION_GRANTED) {
                return customWifiInfo;
            }
            WifiManager wifiManager = (WifiManager) DeviceInfoSDK.getInstance().getContext().getApplicationContext().getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            if (wifiManager == null) {
                return customWifiInfo;
            }
